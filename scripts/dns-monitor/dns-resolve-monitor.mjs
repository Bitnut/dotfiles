...
const intervalMs = Math.max(100, Number(process.env.DNS_MONITOR_INTERVAL_MS ?? 5000));
const reportEveryMs = Math.max(1000, Number(process.env.DNS_MONITOR_REPORT_MS ?? 30000));
const family = Number(process.env.DNS_MONITOR_FAMILY ?? 0);
...
const probeTimer = setInterval(() => {
  runRound().catch((e) => console.error('probe round error:', e));
}, intervalMs);
