...
if [ -z "$ESLINT_ALL" ]; then
    ...
    npx eslint -c ${ESLINT_CONF} --ignore-path ${ESLINT_IGNORE} ./* --fix

    if [ $? -ne 0 ]; then
        echo -e "ERROR: Please check eslint hints.\n"
        npx eslint -c ${ESLINT_CONF} --ignore-path ${ESLINT_IGNORE} ./* -f unix | head -n -2  | awk '{print $NF}' >> eslintReport.log;
        echo -e "Hightest eslint error/warning\n";
        echo -e "times\terror"
        cat ./eslintReport.log | sort | uniq -c | sort -rn | head -n 10
        exit 1
